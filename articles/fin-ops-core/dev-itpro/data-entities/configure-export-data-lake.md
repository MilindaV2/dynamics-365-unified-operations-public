...
> [!NOTE]
> The **Export to Azure Data Lake** feature is currently in lmited preview and may not be available in all regions and environments supported by Microsoft Dynamics 365 Finance and Operations apps. If you are not able to find the **Export to Azure Data Lake** functionality in Life cycle services (LCS) or your Finance and Operations apps, this feature is not currently available in your environment. 

> Our previews are now closed. We hope to enable additional environments in the coming months in several regions. We are accepting requests from customers who would like to join previews in the future. Pl. complete the survey by visiting the link below. Optionally, you can also join a private yammer group by completing the survey below (link to join the yammer group is in the survey). You can stay in touch and ask questions that will help you understand the feature. https://aka.ms/FnODataLakePreviewSurvey 
...
 
> At this time, the **Export to Azure Data Lake** feature is not available in Tier-1 (developer) environments. You need a cloud based Tier-2 or higher environment to enable this feature. You can use the GitHub tools with Tier-1 environments for prototyping and implementation planning.
